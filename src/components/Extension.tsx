...
const Extension:React.FC<ExtensionProps> = ({name, description, isActive, logo, handle_delete, handle_active}:ExtensionProps) => {
    const [Active, setIsActive] = useState(isActive)

    return (
        <article className="flex flex-col border-[1px] border-slate-600 bg-white text-black dark:bg-slate-800 dark:text-white rounded-2xl md:w-[450px] w-full shadow-lg dark:shadow-slate-800 p-2">
            <section className="flex flex-row items-start w-full gap-4">
